import { Typography, Input, Button } from "@material-tailwind/react";
import { useTranslation } from "react-i18next";

export function FooterWithSocialLinks() {
  const { t } = useTranslation();
  const currentYear = new Date().getFullYear();

  const LINKS = [
    {
      title: t("Products"),
      items: [t("Overview"), t("Features"), t("Solutions"), t("Tutorials")],
    },
    {
      title: t("Company"),
      items: [t("About Us"), t("Careers"), t("Press"), t("News")],
    },
    {
      title: t("Resources"),
      items: [t("Blog"), t("Newsletter"), t("Events"), t("Help Center")],
    },
  ];

  return (
    <footer className="relative w-full bg-gradient-to-t from-black via-gray-900 to-black text-white border-t border-gray-800 pt-16">
      <div className="mx-auto max-w-7xl px-8">
        {/* Top Section */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 pb-12">
          {/* Brand Info */}
          <div>
            <Typography variant="h4" className="font-bold mb-4 tracking-wide">
              Shavkatov.M
            </Typography>
            <Typography className="text-gray-400 text-sm leading-relaxed mb-6">
              {t(
                "Building elegant, responsive interfaces with simplicity and beauty. Join us on our journey to modern web design."
              )}
            </Typography>

            {/* Social Icons */}
            <div className="flex gap-4">
              {[
                {
                  icon: (
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path
                        fillRule="evenodd"
                        d="M22 12c0-5.523-4.477-10-10-10S2 6.477 
                        2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797
                        c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 
                        2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 
                        1.562V12h2.773l-.443 2.89h-2.33v6.988
                        C18.343 21.128 22 16.991 22 12z"
                        clipRule="evenodd"
                      />
                    </svg>
                  ),
                  link: "#",
                },
                {
                  icon: (
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path
                        fillRule="evenodd"
                        d="M12.315 2c2.43 0 2.784.013 
                        3.808.06 1.064.049 1.791.218 
                        2.427.465a4.902 4.902 0 011.772 1.153
                        4.902 4.902 0 011.153 1.772c.247.636.416 
                        1.363.465 2.427.048 1.067.06 
                        1.407.06 4.123v.08c0 2.643-.012 
                        2.987-.06 4.043-.049 1.064-.218 
                        1.791-.465 2.427a4.902 4.902 0 
                        01-1.153 1.772 4.902 4.902 0 
                        01-1.772 1.153c-.636.247-1.363.416-2.427.465
                        -1.067.048-1.407.06-4.123.06h-.08
                        c-2.643 0-2.987-.012-4.043-.06
                        -1.064-.049-1.791-.218-2.427-.465
                        a4.902 4.902 0 01-1.772-1.153
                        4.902 4.902 0 01-1.153-1.772
                        c-.247-.636-.416-1.363-.465-2.427
                        -.047-1.024-.06-1.379-.06-3.808v-.63
                        c0-2.43.013-2.784.06-3.808
                        .049-1.064.218-1.791.465-2.427
                        a4.902 4.902 0 011.153-1.772
                        A4.902 4.902 0 015.45 2.525
                        c.636-.247 1.363-.416 
                        2.427-.465C8.901 2.013 
                        9.256 2 11.685 2h.63z"
                        clipRule="evenodd"
                      />
                    </svg>
                  ),
                  link: "#",
                },
                {
                  icon: (
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M8.29 20.251c7.547 0 
                      11.675-6.253 11.675-11.675 
                      0-.178 0-.355-.012-.53A8.348 8.348 
                      0 0022 5.92a8.19 8.19 0 
                      01-2.357.646 4.118 4.118 
                      0 001.804-2.27 8.224 8.224 
                      0 01-2.605.996A4.107 4.107 
                      0 0011.85 9.19a11.65 11.65 
                      0 01-8.457-4.287 4.106 4.106 
                      0 001.27 5.477A4.072 4.072 
                      0 012.8 9.713v.052a4.105 
                      4.105 0 003.292 4.022A4.095 
                      4.095 0 014.24 13.9a4.108 
                      4.108 0 003.834 2.85A8.233 
                      8.233 0 012 18.407a11.616 
                      11.616 0 006.29 1.84" />
                    </svg>
                  ),
                  link: "#",
                },
              ].map(({ icon, link }, i) => (
                <a
                  key={i}
                  href={link}
                  className="text-gray-400 hover:text-white transition-colors duration-300 transform hover:scale-110"
                >
                  {icon}
                </a>
              ))}
            </div>
          </div>

          {/* Links */}
          {LINKS.map(({ title, items }) => (
            <div key={title}>
              <Typography variant="h6" className="font-semibold mb-4 text-white">
                {title}
              </Typography>
              <ul className="space-y-2">
                {items.map((item) => (
                  <li key={item}>
                    <a
                      href="#"
                      className="text-gray-400 hover:text-white transition-colors duration-300"
                    >
                      {item}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          ))}

          {/* Newsletter */}
          <div>
            <Typography variant="h6" className="font-semibold mb-4">
              {t("Newsletter")}
            </Typography>
            <Typography className="text-gray-400 text-sm mb-4">
              {t("Get the latest updates, news, and promotions delivered to your inbox.")}
            </Typography>
            <div className="flex gap-2">
              <Input
                type="email"
                placeholder={t("Enter your email")}
                className="!border !border-gray-700 bg-transparent text-white focus:!border-white"
                labelProps={{ className: "hidden" }}
              />
              <Button ripple={false} className="bg-white text-black hover:bg-gray-200 transition-all duration-300">
                {t("Subscribe")}
              </Button>
            </div>
          </div>
        </div>

        {/* Divider */}
        <div className="border-t border-gray-800 my-6" />

        {/* Bottom Section */}
        <div className="flex flex-col md:flex-row justify-between items-center pb-6 text-gray-500 text-sm">
          <Typography>
            &copy; {currentYear} <span className="text-white">Shavkatov.M</span>. {t("All rights reserved.")} 
          </Typography>
          <div className="flex gap-4 mt-3 md:mt-0">
            <a href="#" className="hover:text-white transition-all">
              {t("Privacy Policy")}
            </a>
            <a href="#" className="hover:text-white transition-all">
              {t("Terms of Service")}
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
}
